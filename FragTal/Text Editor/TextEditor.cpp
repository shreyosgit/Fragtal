...
TextEditor::TextEditor()
	: mFontSize(0.6f) /// Added Font Size
	, mLineSpacing(1.0f)
	, mUndoIndex(0)
	, mTabSize(4)
	, mOverwrite(false)
...
void TextEditor::HandleKeyboardInputs()
{
	...
	ImGui::SetWindowFontScale(mFontSize);
	...
	if (ImGui::IsWindowFocused())
	{
		...
		/// Handling Editor Font Size
		if ((ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Equal)) && io.KeyCtrl || io.MouseWheel > 0.0f && io.KeyCtrl) && mFontSize < 1.99f) {
			mFontSize += 0.05;
			ImGui::SetWindowFontScale(mFontSize);
		}
		else if ((ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Minus)) || io.MouseWheel < 0.0f && io.KeyCtrl) && mFontSize > 0.2f) {
			mFontSize -= 0.05;
			ImGui::SetWindowFontScale(mFontSize);
		}
	}
		...
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					...
					/// Setting the Font for the Error Markers
					ImGui::SetWindowFontScale(mFontSize);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::CPlusPlus()
{
	...
	if (!inited)
	{
		...
		//	"+", "-", "*", "/", "%", "++", "--", "==", "!=", ">", "<", ">=", "<=", "&&", "||", "!", "&", "|", "^", "~", "<<", ">>", "=", "+=", "-=", "*=", "/=", "%=", "&=", "|=", "^=", "<<=", ">>=", ",", "->*", "->", "()", "[]", "::", ".", "?", ":", "new", "delete", "sizeof", "typeid", "const_cast", "dynamic_cast", "reinterpret_cast", "static_cast"
		//};
		...
		//	"{", "}", "[", "]", "#", "##", "(", ")", ";", ":", "...", "_Pragma", "<%", "%>", "<:", ":>", "%:", "%:%:"
		//};
		//for (auto& k : specialSymbols)
		//{
		//	Identifier id;
		//	id.mDeclaration = "Built-in function";
		//	langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		//}
		static const char* const glmKeywords[] = {
			// Vector types
		...
		for (auto& k : glmKeywords)
		{
			Identifier id;
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
		// Functions
		/*static const char* const glmfunctions[] = {
			"radians", "degrees", "sin", "cos", "tan", "asin", "acos", "atan", "sinh", "cosh", "tanh", "asinh", "acosh", "atanh",
			"pow", "exp", "log", "exp2", "log2", "sqrt", "inversesqrt", "abs", "sign", "floor", "trunc", "round", "roundEven", "ceil", "fract", "mod", "modf", "min", "max", "clamp", "mix", "step", "smoothstep", "isnan", "isinf",
			"floatBitsToInt", "floatBitsToUint", "intBitsToFloat", "uintBitsToFloat", "fma", "frexp", "ldexp",
			"length", "distance", "dot", "cross", "normalize", "faceforward", "reflect", "refract",
			"matrixCompMult", "outerProduct", "transpose", "determinant", "inverse",
			"lessThan", "lessThanEqual", "greaterThan", "greaterThanEqual", "equal", "notEqual", "any", "all", "not_"
		};
		for (auto& k : glmfunctions)
		{
			Identifier id;
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
			langDef.mKeywords.insert(k);
